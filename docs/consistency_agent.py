...
is generated (see docs/tech-architecture.md).

Install:
    pip install google-adk

This module defines the three tools and the agent itself. Wire
`run_consistency_check(document_id)` into your FastAPI pipeline after
clause extraction finishes.

NOTE: ADK's Runner/session boilerplate below follows the current ADK 2.x
quickstart pattern. If Antigravity scaffolds a slightly different Runner
signature when it reads the installed google-adk version, let it adjust
this file to match — the tool functions and agent instruction are the
part that matters and should stay as-is.
"""
...
from server.db import supabase  # your existing Supabase client wrapper
from server.embeddings import embed_text, cosine_search  # your existing helpers
...
# to build the tool schema, so keep these accurate and specific.
# ---------------------------------------------------------------------
...
def get_clause_detail(clause_id: str) -> dict:
    ...
    row = (
        ...
        .select("clause_id, section_ref, title, original_text, page_ref, risk_tag")
...
# ---------------------------------------------------------------------
# Orchestration — runs the agent once per flagged clause in a document
# ---------------------------------------------------------------------
...
async def run_consistency_check(document_id: str) -> list[dict]:
    ...
    Returns the list of inconsistencies found (also persisted to the
    `inconsistencies` table by the agent's own tool calls).
    """
    clauses = (
        supabase.table("clauses")
        .select("clause_id, section_ref, title, original_text, risk_tag")
    ...
    for clause in clauses:
        ...
        session = await session_service.create_session(
            app_name="clarity", user_id="system", session_id=f"{document_id}-{clause['clause_id']}"
        )

        prompt = (
            f"Document: {document_id}\n"
            f"Clause under review: {clause['clause_id']} "
